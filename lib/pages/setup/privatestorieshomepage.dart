...
class PrivateHomePage extends StatefulWidget {
  const PrivateHomePage({Key key, this.user}) : super(key: key);
...
class _PrivateHomePageState extends State<PrivateHomePage> {
  ...
  fetchDatabaseList() async {
    ...
    dynamic privateStories = await DataMethods().getUsersPrivateStories();
  ...
  Widget continueButton(BuildContext context, String id,viewed) {
    return FlatButton(
      child: Text("Delete"),
      onPressed: () {
    ...
    return showDialog(
        ...
        builder: (BuildContext context) {
          return AlertDialog(
            ...
            actions: [cancelButton(context), continueButton(this.context, id, viewed)],
          );
        });
  }
  ...
  deleteStory(String id, bool viewed) async {
    try {
      ...
      await FirebaseFirestore.instance
          ...
          .doc(userId)
          .collection('story')
          .doc(userId).collection(whereisIt).doc(id)
          .delete();
      fetchDatabaseList();
      ShowToast('Sucessfully Deleted Story');
            ...
            Container(
              padding: EdgeInsets.symmetric(vertical: 10),
                  ...
                  onTap: () => Navigator.of(context).push(MaterialPageRoute(
                      builder: (context) =>
                          EditStory(userPublicStories[index], false)))),
            ),
            Container(
              padding: EdgeInsets.symmetric(vertical: 10),
                          child: IconSlideAction(
            ...
            : Container(
                ...
                child: Column(
                  ...
                  children: [
                    Container(
                      child: Container(
                        child: userPublicStories.length > 0
                            ? Container()
                            : Expanded(
                              child: Column(
                                crossAxisAlignment: CrossAxisAlignment.center,
